...
export function DiaryCard({ diary, visitorCount = 0, incidentCount = 0 }: DiaryCardProps) {
  ...
  const descriptionPreview = diary.description.length > 150 
    ? diary.description.substring(0, 150) + '...' 
    : diary.description

  return (
    <Link href={`/diary/${diary.id}`} 
      className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-2xl h-full w-full flex">
      
      <Card 
        className="transition-all duration-200 cursor-pointer h-full w-full flex flex-col hover:-translate-y-1 hover:shadow-md motion-reduce:transition-none"
      >
